...
def handle_best_buy_country_gate(page, search_url: str) -> None:
    ...
    if not looks_like_country_gate:
        return
    ...
    try:
        page.get_by_role("link", name=re.compile(r"united states", re.I)).click(timeout=6000)
        clicked = True
    except Exception:
        pass
    ...
    if not clicked:
        try:
            page.get_by_role("button", name=re.compile(r"united states", re.I)).click(timeout=6000)
            clicked = True
        except Exception:
            pass
    ...
    if not clicked:
        try:
            page.get_by_text(re.compile(r"united states", re.I)).click(timeout=6000)
            clicked = True
        except Exception:
            pass
        ...
        rows.append(
            {
                ...
                "status": "ok",
                "method": "v4_parallel_visible_dom",
            }
        )
...
def run_parallel_pairs(
    ...
    print("")
    print(f"[INFO] Running {len(pairs)} parallel-safe retailers with MAX_WORKERS={MAX_WORKERS}")

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_map = {
            executor.submit(scrape_retailer_worker, retailer, keyword): (retailer, keyword)
        ...
        for future in as_completed(future_map):
            ...
            try:
                ...
                print(f"[ERROR] Worker crashed for {name}: {e}")
                ...
                rows = [
                    {
                        ...
                        "product_url": "",
                        "image_url": "",
                        "source_url": format_url(retailer, keyword),
                        "status": f"worker_error: {e}",
                        "method": "v4_worker_exception",
                    }
                ]
            ...
            print(f"[INFO] Saved progress after {name}. Total rows: {len(all_rows)}")


def run_sequential_pairs(
    pairs: list[tuple[dict, str]],
    all_rows: list[dict[str, str]],
    output_csv: Path,
) -> None:
...
def run_once() -> None:
    ...
    parallel_pairs = [
        (retailer, keyword)
        for retailer, keyword in pairs
        if retailer.get("parallel_ok", True)
    ]

    sequential_pairs = [
        (retailer, keyword)
        for retailer, keyword in pairs
        if not retailer.get("parallel_ok", True)
    ]

    output_csv = OUTPUT_DIR / "retailer_championship_prices_v4.csv"
    all_rows: list[dict[str, str]] = []
    ...
    print("")
    print("[DONE] V4 championship run finished")
